...
/**
 * InteractableObject class represents object such as desk which player can interact with<br>
 * They can contain hidden items or perform special action after player uses specific item while
...
class InteractableObject {
   ...
   *     UNLOCK_DOOR_TO_ROOM)
   * @param roomToUnlockAfterItemUsage item name with name of room which will be opened after item
   *     usage
   * @param itemsToAddToRoomAfterItemUsage item name with list of items which will be added to room
   *     after player uses this item
   * @param outputTextAfterItemUsage item name with output text, this text will inform player about
   *     performed action (e.q. "You just unlocked door to...")
   */
   ...
   * InteractableObject the player stands nearby (e.g. unlock door to another room, give extra items
   * to player inventory)
   *
  ...
  public String performSpecialAction(String itemName, GamePlan gamePlan) {
    String outputText;
    String actionType;
    if (itemsWithActionTheyPerform == null || itemsWithActionTheyPerform.get(itemName) == null) {
      actionType = "";
    } else {
  ...
  private String unlockDoorToRoom(String itemName, GamePlan gamePlan) {
    ...
    if (roomToUnlock != null) {
      if (!roomToUnlock.equals(RoomName.getRoomName("RB_202"))) {
        gamePlan.getActualRoom().getNeighboringRoomByName(roomToUnlock).setIsRoomLocked(false);
      }
