 ...
 * objects on non-player characters<br>
 * This class provides information about which game room will be unlocked, or which items will be
 * added to actual game room after player uses specific item from his inventory while standing next
 * to interactable object or non-player character
 */
public enum SpecialActions {
  ...
  COUGHING_TEACHER(
      NonPlayerCharacterName.getNonPlayerCharacterName("COUGHING_TEACHER"), null, null, null, null),
  IT_ADMIN(
  ...
  static {
    for (SpecialActions s : values()) {
      OBJECTS_AND_NON_PLAYER_CHARACTERS_WITH_SPECIAL_ACTIONS.put(s.getInteractableObjectName(), s);
    }
  }
