 ...
 * interactable object<br>
 * Player can find these hidden items by performing ExecuteItem or ExecuteObject commands
 *
...
class HiddenItems {
   ...
   * between items which are already in room<br>
   * (e.g. takes item "bag" from room, finds hidden item "pen" and then moves this item between
   * other items which are already in room)
   *
  ...
  /**
   * Method takes item from player's inventory, found all items which it hides and move these found
   * items between items which are already in room<br>
   * (e.g. takes item "wallet" from player's inventory, find hidden item "money" and then moves this
   * item between other items which are already in room)
   *
   * @param gamePlan actual state of game plan to perform changes on
  ...
  /**
   * Method takes interactable object from room, found all items which it hides and move these found
   * items between items which are already in room<br>
   * (e.g. takes object "desk" from actual room, find hidden item "screwdriver" and then moves this
   * item between other items which are already in room)
   *
   * @param gamePlan actual state of game plan to perform changes on
  ...
  /**
   * Method return names of all items inside the player's inventory with weight for each one of them
   * <br>
   * Items with their weights are returned as one String, where each entry is separated by comma and
   * max. 4 items with their weight are displayed per line of String
   *
