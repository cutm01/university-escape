...
class DropCommand implements Command {
  ...
  /**
   * Method is used to drop one or more items from player inventory, which are added to actual room
   * then and player can take them again later in game
   *
  ...
  public String executeCommand(String... commandParameters) {
    ...
    for (String itemName : commandParameters) {
      if (inventory.isItemInInventory(itemName)) { // to prevent drop one SAME item multiple times
