...
class UseCommand implements Command {
  ...
  /**
   * Method is used to perform special action with game items<br>
   * These items can be used while standing next to interactable object or NPC in actual game room
   *
  ...
  public String executeCommand(String... commandParameters) {
    ...
    if (itemToUseName.equals(ItemName.getItemName("MEDICAL_MASK"))) {
      ...
      boolean isPlayerWearingMedicalSuit = gamePlan.getPlayer().getIsPlayerWearingMedicalSuit();
    ...
    if (isPlayerStandingNextToNPC) {
      ...
      return gamePlan.getActualNonPlayerCharacter().performSpecialAction(itemToUseName, gamePlan);
    }
    ...
    if (isPlayerStandingNextToInteractableObject) {
      setAlternativeGameEnding(itemToUseName);
      return gamePlan.getActualInteractableObject().performSpecialAction(itemToUseName, gamePlan);
    }
  ...
  private void setAlternativeGameEnding(String itemName) {
    ...
    if (itemName.equals(ItemName.getItemName("ROPE"))
      ...
      if (NonPlayerCharacterName.getNonPlayerCharacterName("IT_ADMIN").equals(playerStandsBy)) {
        gamePlan.getPlayer().setHasPlayerAttackedITAdmin(true);
      }
      if (NonPlayerCharacterName.getNonPlayerCharacterName("DOOR_KEEPER").equals(playerStandsBy)) {
    ...
    if (itemName.equals(ItemName.getItemName("STOLEN_MONEY"))
        && NonPlayerCharacterName.getNonPlayerCharacterName("DOOR_KEEPER").equals(playerStandsBy)) {
