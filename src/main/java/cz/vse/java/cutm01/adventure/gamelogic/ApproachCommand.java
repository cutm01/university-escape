...
class ApproachCommand implements Command {
   ...
   *
   * @param commandParameters max 1 name of interactable object or non-player character player wants
   *     to approach
  ...
  public String executeCommand(String... commandParameters) {
    ...
    if (commandParameters.length > 1) {
      return "No tak, spomaľ trochu, a vyber si iba jednu osobu alebo objekt, ku ktorému chceš pristúpiť!\n"
    ...
    if (gamePlan.getActualRoom().isInteractableObjectInRoom(approachTo)) {
      InteractableObject object = gamePlan.getActualRoom().getInteractableObjectByName(approachTo);
