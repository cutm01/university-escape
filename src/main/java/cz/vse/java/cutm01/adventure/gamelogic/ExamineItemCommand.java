 ...
 * inventory or from actual room in more detail.<br>
 * Player can find hidden items this way
 *
...
class ExamineItemCommand implements Command {
   ...
   * "wallet" or "keys")<br>
   * Found items are transferred to room's items and player can take them later
   *
  ...
  public String executeCommand(String... commandParameters) {
    ...
    if (isItemInRoom) {
      boolean wasItemAlreadyExamined =
          gamePlan.getActualRoom().getItemByName(itemToExamineName).getWasItemAlreadyExamined();
      if (!wasItemAlreadyExamined) {
  ...
  /**
   * Method examines item from player's inventory and informs him about found hidden items, if there
   * are any<br>
   * Examined item is marked as "wasAlreadyExamined == true" to avoid future calls to this method,
   * found items are transferred to room's items and player can take them later
   *
   * @param itemToExamineName name of item player wants to examine
   * @return String which informs player about result of item examination (i.e. if any hidden items
   *     was found)
   */
  private String examineItemFromInventory(String itemToExamineName) {
    ...
    itemToExamine.getHiddenItems().moveHiddenItemsFromInventoryToRoom(gamePlan, itemToExamineName);
  ...
  /**
   * Method examines item from actual room and informs player about found hidden items, if there are
   * any<br>
   * Examined item is marked as "wasAlreadyExamined == true" to avoid future calls to this method,
   * found items are transferred to room's items and player can take them later
   *
