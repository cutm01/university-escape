...
public class FormatOutputTextUtils {
    ...
    /**
     * Method return names and weights of items which will be taken from actual game room<br> Items
     * with their weights are returned as one String, where each entry is separated by comma and
    ...
    public static String getOutputTextForTakenItems(
        ...
        while (itemIterator.hasNext()) {
            outputText
                .append(actualGameRoom.getItemByName(itemIterator.next()).getItemWithItsWeight());
     ...
     * and max. maxItemNamesPerLine items with their weights are displayed per line of output
     * String
     *
    ...
    /**
     * Method return names of non-valid items which cannot be taken from room or dropped from
     * player's inventory, e.g. item's name was misspelled or item is not in player's inventory<br>
     ...
     *
     * @param nonValidItemsNames  list with names of non-valid items which cannot be dropped or
     *                            taken
     * @param maxItemNamesPerLine how many item names per line will be printed
